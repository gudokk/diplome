    ...
    <div className="flex-1 flex flex-col justify-between">
      <div>
        ...
        <div className="text-sm text-gray-700 mb-1">
          <span className="font-medium">Высота:</span> {resort.min_height} -{" "}
          {resort.max_height} м
        </div>
        <div className="text-sm text-gray-700 mb-1">
        ...
        <div className="text-sm text-gray-700">
          <span className="font-medium">Скипасс:</span>{" "}
          {resort.price_day ? `${resort.price_day} ₽ в день` : "нет данных"}
        </div>
      </div>
      <div className="flex justify-between items-end mt-2">
        ...
        {resort.latest_review && (
          <p className="text-sm text-gray-600 max-w-[300px] truncate">
            <Link to={`/resorts/${resort.id}/reviews`} className="font-medium ">
              {resort.latest_review}
            </Link>
          </p>
        )}
...
const ResortsSelector = () => {
  ...
  const sortedResorts = [...resorts].sort((a, b) => {
    ...
    switch (selectedSort) {
      ...
      case "price":
        return (
          multiplier * ((a.price_day || Infinity) - (b.price_day || Infinity))
        );
      case "trail":
  ...
  return (
    <div>
      ...
      <p className="text-gray-700 text-left p-10 pb-2">
        ...
        {resorts.length === 1
          ? ""
          : resorts.length < 5 && resorts.length === 1
          ? "а"
          : "ов"}
      </p>
      ...
      <div className="flex flex-wrap gap-2 px-10 pb-6">
        {sortButtons.map(({ key, label }) => (
          ...
          >
            {label}
            {selectedSort === key && <span>{ascending ? "↑" : "↓"}</span>}
          </button>
        ))}
      </div>
        ...
        <label className="text-sm block text-black">
          <input
            ...
            onChange={(e) =>
              setWeatherFilters({
                ...weatherFilters,
                snow_last_3_days: e.target.checked,
              })
            }
          />{" "}
        ...
        <label className="text-sm block text-black ">
          <input
            ...
            onChange={(e) =>
              setWeatherFilters({
                ...weatherFilters,
                snow_expected: e.target.checked,
              })
            }
          />{" "}
