...
const NewPage = () => {
  ...
  const handleSubmitComment = async () => {
    ...
    if (res.ok) {
      const updated = await fetch(`/back/api/comments/${id}`).then((r) =>
        r.json()
      );
      setComments(updated);
      setNewComment("");
  ...
  return (
    <div className={styles["new-page"]}>
      ...
      <div className="min-h-screen flex items-center justify-center p-4 flex-col">
            ...
            <div className="mb-3 -mx-4 sm:mx-0 sm:rounded-lg overflow-hidden">
              {news.image && (
                <img
                  ...
                  alt={news.title}
                  className="w-full max-w-md sm:max-w-2xl mx-auto mb-4 rounded-lg object-cover px-4 sm:px-0"
                />
              )}
        ...
        {isAuthenticated && (
          <div className="mt-10 w-full p-4 max-w-6xl mx-auto px-2 sm:px-4 bg-[#d9dde9] border rounded-lg shadow-sm">
